...
<script>
	...
	import { $, $$, ajax, attr, offset, on, Promise, startsWith, ucfirst } from 'uikit/src/js/util'
			...
			$route: {
				handler() {
					...
					new Promise((resolve, reject) => {
						...
						ajax(`/pages/${page}.md`).then(({ response }) => {
...
`
						...
						function() { this.error = 'Failed loading page' },
					)
				},
				immediate: true,
			},
		...
		mounted() {
			...
			on(this.$refs.container, 'click',
				`a:not([href^='http']):not([href^='#']):not([href^='/']):not([href^='../']):not([href^='images/'])`,
				e => {
					e.preventDefault()
					this.router.replace(e.target.pathname + e.target.hash)
				})
			on(window, 'click', `a[href^='#']:not([href='#'])`, e => {
				!e.defaultPrevented && history.pushState({}, '', e.target.href)
			})
			on(window, 'popstate', () => {
				setTimeout(() => {
